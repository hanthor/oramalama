...
// SelectItem represents an option in a TUI selector.
...
// renderContent renders the selector content (title, items, help text).
...
func cursorForCurrent(items []SelectItem, current string) int {
	...
	// Prefer exact name matches before tag-prefix fallback
...
func (m multiSelectorModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case tea.KeyRunes:
			// On some terminals, space arrives as KeyRunes instead of KeySpace.
...
// SelectMultiple shows a bubbletea multi-selection picker.
// Returns the selected item names, or an error if cancelled.
