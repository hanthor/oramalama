...
// TestIntegration_CodingToolConfig tests dry-run config for every coding tool.
func TestIntegration_CodingToolConfig(t *testing.T) {
	...
	for _, tool := range tools {
		t.Run(tool.name, func(t *testing.T) {
			if tool.checkBin != "" {
				if _, err := exec.LookPath(tool.checkBin); err != nil {
					t.Skipf("%s not installed", tool.checkBin)
				}
			}
			out, err := exec.CommandContext(ctx, binary,
				"--dry-run", "--model", integrationModel,
				"launch", "--tool", tool.name,
			).Output()
			if err != nil {
				t.Fatalf("launch %s: %v\noutput: %s", tool.name, err, string(out))
			}
			if !strings.Contains(strings.ToLower(string(out)), tool.name) && tool.name != "vscode" {
				t.Errorf("expected %s reference: %s", tool.name, string(out))
			}
			t.Logf("%s config: %s", tool.name, string(out))
		})
	}
}
...
// TestIntegration_GooseLaunch tests goose launch against a running server.
func TestIntegration_GooseLaunch(t *testing.T) {
	checkRamalama(t)
	ctx, cancel := context.WithTimeout(context.Background(), integrationTimeout)
	defer cancel()
	pullModel(ctx, t)
	...
	// Start server first.
	startCmd := exec.CommandContext(ctx, binary,
		"--model", integrationModel,
		"launch", "--tool", "server",
	)
	startOut, _ := startCmd.Output()
	t.Logf("server start: %s", startOut)

	// Test launch with dry-run for goose.
	t.Run("goose_dryrun", func(t *testing.T) {
		out, err := exec.CommandContext(ctx, binary,
			"--dry-run", "--model", integrationModel,
			"launch", "--tool", "goose",
		).Output()
		if err != nil {
			t.Fatalf("launch goose dry-run: %v\noutput: %s", err, string(out))
		}
		if !strings.Contains(string(out), "goose") {
			t.Errorf("expected goose reference: %s", string(out))
		}
		t.Logf("goose dry-run: %s", string(out))
	})

	// Test real goose launch if goose is installed.
	if _, err := exec.LookPath("goose"); err == nil {
		t.Run("goose_real", func(t *testing.T) {
			cmd := exec.CommandContext(ctx, binary,
				"--model", integrationModel,
				"launch", "--tool", "goose", "--prompt", "say hi",
			)
			out, err := cmd.Output()
			if err != nil {
				t.Logf("goose launch (may need config): %v\noutput: %s", err, string(out))
				return
			}
			t.Logf("goose launch output: %s", string(out))
		})
	}
}

// TestIntegration_AllToolConfigs tests dry-run config for every coding tool.
func TestIntegration_AllToolConfigs(t *testing.T) {
	checkRamalama(t)
	ctx, cancel := context.WithTimeout(context.Background(), integrationTimeout)
	defer cancel()
	pullModel(ctx, t)
	...
	tools := []string{"pi", "opencode", "goose", "vscode"}
	for _, tool := range tools {
		t.Run(tool, func(t *testing.T) {
			...
			if !strings.Contains(strings.ToLower(string(out)), tool) && tool != "vscode" {
				t.Errorf("expected %s reference: %s", tool, string(out))
			}
			t.Logf("%s config: %s", tool, string(out))
		})
	}
}
